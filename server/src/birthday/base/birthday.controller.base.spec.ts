...
describe("Birthday", () => {
  ...
  test("GET /birthdays/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/birthdays"}/${nonExistingId}`)
      .expect(404)
      .expect({
        statusCode: 404,
  ...
  test("GET /birthdays/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/birthdays"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
